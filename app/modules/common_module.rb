...
module CommonModule
  ...
  def define_query_params(query_params, client = false, is_foreign = false)
    ...
    if client && client['role'] == Roles::CLIENT && is_foreign
      filter['client_id'] = BSON::ObjectId.from_string(client['id'])
    end

    if client && client['role'] == Roles::CLIENT && !is_foreign
      filter['_id'] = BSON::ObjectId.from_string(client['id'])
    end
